...
//#define BMA2XXX_ENABLE_INT1  //guliangzeng mask for INT 20131115
...
#ifdef CONFIG_OF
static int bma2xx_power_init(struct bma2xx_data *data, bool on)
{
        int rc;
        int vol_uv;
	int enabled = 0; 
	...
	if (!on) {
		...
		regulator_set_voltage(data->vdd, 0, BMA2XX_VDD_MAX_UV);
		regulator_set_voltage(data->vio, 0, BMA2XX_VIO_MAX_UV);
	} else {
		if (!(data->vdd)){
			data->vdd = regulator_get(&data->bma2xx_client->dev, "vdd");
			if (IS_ERR(data->vdd)) {
				rc = PTR_ERR(data->vdd);
				dev_err(&data->bma2xx_client->dev, "Regulator get failed vdd rc=%d\n", rc); 
				return rc;
			}    
		}
		vol_uv = regulator_get_voltage(data->vdd);
		enabled = regulator_is_enabled(data->vdd);
		dev_info(&data->bma2xx_client->dev, "vdd current voltate is %duv, enabled = %d ", vol_uv, enabled);
		if (((vol_uv <= BMA2XX_VDD_MIN_UV) || (vol_uv >= BMA2XX_VDD_MAX_UV))
				&& (regulator_count_voltages(data->vdd) > 0)) {
			rc = regulator_set_voltage(data->vdd, BMA2XX_VDD_MIN_UV,
					BMA2XX_VDD_MAX_UV);
			if (rc) {
				dev_err(&data->bma2xx_client->dev,
		...
		if (!(data->vio)){
			data->vio = regulator_get(&data->bma2xx_client->dev, "vio");
			if (IS_ERR(data->vio)) {
				rc = PTR_ERR(data->vio);
				dev_err(&data->bma2xx_client->dev,
		...
		dev_info(&data->bma2xx_client->dev, "vio current voltate is %duv, enabled = %d \n", vol_uv, enabled);
		if (((vol_uv <= BMA2XX_VIO_MIN_UV) || (vol_uv >= BMA2XX_VIO_MAX_UV))
				&& (regulator_count_voltages(data->vio) > 0)) {
			rc = regulator_set_voltage(data->vio, BMA2XX_VIO_MIN_UV,
					BMA2XX_VIO_MAX_UV);
			if (rc) {
				dev_err(&data->bma2xx_client->dev,
				  ...
				  const char *buf, size_t count)
{
	...
	dev_info(&client->dev, "set delay to %ldms\n", data);
	return count;
}
...
static void bma2xx_set_enable(struct device *dev, int enable)
{
	...
	dev_info(&client->dev, "set enable from %d to %d\n", pre_enable, enable);
	
	mutex_lock(&bma2xx->enable_mutex);
	if (enable) {
		if (pre_enable == 0) {
			bma2xx_power_init(bma2xx, true);
		...
		if (pre_enable == 1) {
			...
			bma2xx_power_init(bma2xx, false);
		}
	}
				     ...
				     const char *buf, size_t count)
{
	...
	struct i2c_client *client = to_i2c_client(dev);
	struct bma2xx_data *bma2xx = i2c_get_clientdata(client);
/*guliangzeng modify for compile error 201131114 start*/
	int error;
        //data = kstrtoul(buf, NULL, 10);
	error = kstrtoul(buf, 10, &data);
	if (error)
		return error;
/*guliangzeng modify for compile error 201131114 end*/
...
/* jinfeichao modified for CTS start */
...
/* jinfeichao modified for CTS end */
...
#ifdef CONFIG_OF
static int bma2xx_parse_gpio(struct i2c_client *client)
{
	struct device_node *np = client->dev.of_node;
	u32 temp_val;
	int rc;

	temp_val = of_get_named_gpio(np, "bma2xx,gpio-irq1", 0);
	if (gpio_is_valid(temp_val)) {
		rc = gpio_request(temp_val, "bma2xx_irq1");
		if (rc) {
			dev_err(&client->dev, "irq1 gpio request failed");
		} else   
			gpio_direction_input(temp_val);
		client->irq = temp_val;
	}   		

	temp_val = of_get_named_gpio(np, "bma2xx,gpio-irq2", 0);
	if (gpio_is_valid(temp_val)) {
		rc = gpio_request(temp_val, "bma2xx_irq2");
		if (rc) {
			dev_err(&client->dev, "irq2 gpio request failed");
		} else   
			gpio_direction_input(temp_val);
	}    
	return 0;
			...
			const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(client->adapter, I2C_FUNC_I2C)) {
		dev_err(&client->dev, "[bma2xxx]i2c_check_functionality error\n");
		goto exit;
	}
	...
	if (client->dev.of_node) {
		bma2xx_power_init(data, true);
		bma2xx_parse_gpio(client);
		err =of_property_read_u32(client->dev.of_node, "bma2xx,orientation", &val);
		if (err)
			dev_err(&client->dev, "get orientation fialed\n");
		else
	...
	if (err < 0) {
		dev_err(&client->dev, "read chip id error, maybe not connected\n");
		goto power_deinit;
	}
	chipid = err;
	pr_info("bma2xx chip id is 0x%x\n", chipid);
	...
	***************/
	...
	if (!data->input) {
		dev_err(&client->dev, "allocate input device fail\n");
		goto power_deinit;
	}
	data->input->name = "zte_acc";
	data->input->id.bustype = BUS_I2C;
	data->input->dev.parent = &data->bma2xx_client->dev;

	input_set_capability(data->input, EV_REL, LOW_G_INTERRUPT);
	input_set_capability(data->input, EV_REL, HIGH_G_INTERRUPT);
	input_set_capability(data->input, EV_REL, SLOP_INTERRUPT);
	input_set_capability(data->input, EV_REL, DOUBLE_TAP_INTERRUPT);
	input_set_capability(data->input, EV_REL, SINGLE_TAP_INTERRUPT);
	input_set_capability(data->input, EV_ABS, ORIENT_INTERRUPT);
	input_set_capability(data->input, EV_ABS, FLAT_INTERRUPT);
	input_set_abs_params(data->input, ABS_X, ABSMIN, ABSMAX, 0, 0);
	input_set_abs_params(data->input, ABS_Y, ABSMIN, ABSMAX, 0, 0);
	input_set_abs_params(data->input, ABS_Z, ABSMIN, ABSMAX, 0, 0);
	set_bit(EV_REL, data->input->evbit);
	set_bit(ABS_X, data->input->relbit);
	set_bit(ABS_Y, data->input->relbit);
	set_bit(ABS_Z, data->input->relbit);
...
#endif
	printk("bma2xx probe success\n");
	return 0;

error_sysfs:
	input_unregister_device(data->input);
power_deinit:
	bma2xx_power_init(data, false);
	kfree(data);
exit:
	printk("bma2xx probe failed\n");
	return err;
}
...
{
        if(0xff == chipid) {
                printk("not create bma2xx proc file due to unconnected\n");
                return 0;
        }    
...
module_i2c_driver(bma2xx_driver);
